...
const SkillOverviewMobile: FC<SkillOverviewProps> = ({ skillData }) => {
  ...
  const currentCategory = useMemo(() => {
    const category = Object.entries(skillData)
      .find((entry: [string, Skill[]]) => entry[1]
        .some((skill) => skill.value === itemArray[currentItem].value))![0];

    return (
      <span
        className="rounded outline-none duration-500 px-4 py-2 text-center w-1/2"
      >
